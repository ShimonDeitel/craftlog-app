    ...
    private func entryRow(_ entry: ProjectEntry) -> some View {
        VStack(alignment: .leading, spacing: 4) {
            Text(entry.title)
                .font(Theme.headingFont)
                .foregroundColor(Theme.textPrimary)
            Text(String(describing: entry.pattern))
...
struct AddEntrySheet: View {
    ...
    @State private var draftYarn: String = ""
        ...
        NavigationStack {
            Form {
                Section("Details") {
                    ...
                    TextField("Yarn", text: $draftYarn)
                        .accessibilityIdentifier("field_yarn")
            ...
            .navigationTitle(entry == nil ? "Add Project" : "Edit Project")
            .toolbar {
    ...
    private func save() {
        if var existing = entry {
            ...
            let newEntry = ProjectEntry(title: title, rating: rating, pattern: draftPattern, yarn: draftYarn, status: draftStatus, notes: draftNotes)
            store.add(newEntry)
        }
        draftPattern = ""
        draftYarn = ""
